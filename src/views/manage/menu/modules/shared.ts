/**
 * 菜单 component / routePath 拆装工具（完整移植自 Soybean example 菜单模块）。
 *
 * `component` 形如 `layout.base$view.manage_user`：`$` 拆 layout / page 两段；
 * layout 段以 `layout.` 前缀、page 段以 `view.` 前缀。
 * `routePath` 形如 `/manage/user/:id`：尾部 `/:param` 单独成段。
 */
