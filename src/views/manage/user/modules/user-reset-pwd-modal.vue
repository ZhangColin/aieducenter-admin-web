...
const title = computed(() => `${$t('page.manage.user.resetPwd')}${props.rowData?.username ? ` — ${props.rowData.username}` : ''}`);

const newPasswordRef = computed(() => model.value.newPassword);

const rules = {
  newPassword: [defaultRequiredRule, { pattern: REG_PWD, message: $t('page.manage.user.pwdRule'), trigger: 'change' }],
