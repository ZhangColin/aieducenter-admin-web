...
- **✅ spec 已发布（2026-08-01，via `/to-spec`）**：[#20](https://github.com/ZhangColin/aieducenter-admin-web/issues/20)（`ready-for-agent`）= 动态菜单（dynamic 路由模式）前端实现。测试 seam = 手动 E2E（同 spec #1/#13）。**阻塞边 = [REQ-13](https://github.com/ZhangColin/aieducenter-admin/issues/20)（✅ 后端已交付实测，2026-08-02，见「需服务端支持」REQ-13 条目）**；不依赖后端的（`isStaticSuper` 修复、转换器、service 三适配）可在 static 模式下先合入、零行为变化，翻 env 是最后一步。已 grep 核实无任何代码消费 `/auth/current.menus`——后端删字段无切换协调风险。**✅ 已拆 3 ticket（2026-08-02，via `/to-tickets`，全部 `ready-for-agent`）**：[#21](https://github.com/ZhangColin/aieducenter-admin-web/issues/21) T1 dynamic 前置适配（frontier，static 零行为变化）｜[#22](https://github.com/ZhangColin/aieducenter-admin-web/issues/22) T2 动态路由闭环（翻 env，blocked by #21）｜[#23](https://github.com/ZhangColin/aieducenter-admin-web/issues/23) T3 非超管裁剪与配置行为全矩阵 + 菜单页 toast「更新成功，刷新页面后导航生效」（blocked by #22）。下一步 `/implement` 从 #21 起，清 context 逐个做。
  ...
  - **✅ #23 已交付（2026-08-02，commit `543ed31`）**：① 菜单页提交 toast 通用「更新成功/新增成功」→ 专属「更新成功，刷新页面后导航生效」（新键 `page.manage.menu.updateSuccess`，手写 Schema 三处同步；**add/edit 提交同一文案**——spec 规定单键单文案，双轴 review 指出 add 场景「更新成功」语义错位，按 spec 字面保留、留作者定夺是否拆第二键）；② 非超管全矩阵 E2E 绿（playwright-core headless chromium 驱动，脚本在 /tmp 不入仓）：curl 造角色（菜单子集 首页+系统管理>用户管理 + home=manage_user）+ 用户 → 登录落 /manage/user、导航只见分配菜单、/manage/role|menu 直达 403、/nonexistent 404、`/`→/manage/user；角色 home 置 null → 兜底落第一个可见叶子 /home；禁用叶子 manage_user → F5 导航消失 + 直达 403（空目录 manage 一并消失）；禁用 directory manage → 子树整体消失；i18nKey 缺键菜单导航显示 menuName；hideInMenu 导航不可见 URL 可达；sortOrder 0 升最前 + 换图标 F5 反映；toast zh/en 双验；env 翻 static 静态导航恢复 → 翻回 dynamic；测试数据全清（含 DB 清孤儿关联，见 REQ-14）。typecheck/lint 干净。
    - **⚠️ 新发现（E2E 踩坑，非 bug）——菜单 routeName 的 `_` 是层级分隔符**：elegant-router 约定 routeName 以下划线编码父子层级（`manage_user`=manage 的子路由）；**顶级菜单 routeName 含 `_` 会被当作不存在的父路由的子路由而静默丢弃**（导航可见但 URL 直达 404——路由根本没注册，实测 `router.getRoutes()` 无该条）。本轮自建的 `e2e_no_i18n`/`e2e_hidden` 即中招，改 `e2epage`/`e2ehidden` 后正常。设计内用法（新菜单 = 前端构建中已存在页面、routeName 取生成路由名）天然规避；但菜单管理页 routeName 是自由输入，维护人员可踩同样坑——后续可考虑在菜单页加校验提示（未立项）。
...
- **[REQ-14] 角色删除「使用中」检查未过滤已软删用户 → 角色无法删除 → 🐞 需求文档已备（2026-08-02，#23 E2E 数据清理发现；状态：待提后端 issue）** — 用户软删后其 `sys_admin_user_roles` 关联记录残留，`AdminRoleRepository.isUsedByAnyAdmin`（`AdminRoleRepository.java:52`）只按 `roleId` 计数、未 join 用户过滤 `deleted=false` → 挂过（已删）用户的角色 `DELETE /roles/{id}` 永返 403 `ADMIN_013 角色正在使用中`。与 REQ-5 同类（软删过滤）不同路径。修复倾向：查询侧 join `AdminUser` 加 `deleted=false`（同 `findByIdInAndDeletedFalse` 显式过滤惯例），或用户软删级联清关联。本次清理靠直连 DB 删孤儿关联完成。需求详情 + 复现脚本见 `docs/backend-requirements/REQ-14-role-delete-in-use-ignores-softdeleted-users.md`。
